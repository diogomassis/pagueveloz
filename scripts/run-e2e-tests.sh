...
echo "Bringing up all services (api + infra)..."
docker compose up -d --build

# Quick check for RabbitMQ .erlang.cookie permission error and retry once with fresh volume
sleep 2
if docker compose logs rabbitmq --no-color | grep -q "\.erlang.cookie: eacces"; then
  echo "Detected RabbitMQ .erlang.cookie permission error — retrying after removing rabbitmq_data volume"
  docker compose down --volumes --remove-orphans >/dev/null 2>&1 || true
...
echo "Waiting for API health on http://localhost:5000/health (timeout ${WAIT_TIMEOUT}s)..."
elapsed=0
while true; do
  ...
  sleep 1
# Wait for RabbitMQ TCP port on localhost:5672 to be ready
...
function request_and_expect() {
  ...
  status=$(curl -s -o "$TMPRESP" -w "%{http_code}" -H "Content-Type: application/json" -X "$method" "http://localhost:5000$path" -d "$data")
  echo "HTTP $status"
